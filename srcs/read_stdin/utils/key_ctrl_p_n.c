...
/*   Updated: 2021/04/03 18:30:18 by tjinichi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_down_one_line(char *buf, t_string *command, t_minishell *info)
{
	...
	if (command->len + info->window.prompt_len > info->key.save_command_len)
	{
		if (ft_putstr_fd(info->key.down, STDIN_FILENO) == false)
			all_free_exit(info, ERR_WRITE, __LINE__, __FILE__);
		moving(info->key.save_command_len + info->window.prompt_len - info->window.ws.ws_col, info->key.left, info);
