...
/*   Updated: 2021/04/22 18:16:37 by tjinichi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_minishell *info)
{
	...
	insert_index = get_now_index(info, command);
	len = command->len + 1;
									...
									t_minishell *info)
{
	...
	while (buf[++i])
	{
		...
		cur_index = get_now_index(info, command);
		...
		putstr_fd(command->str + cur_index + 1, STDIN,
