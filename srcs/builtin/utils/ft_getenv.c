...
char		*ft_getenv(char *str, t_envlst *env_lst, bool get_shell_val)
{
	...
	if (get_shell_val && g_global.info.unset_pwd_flag == false && ft_strcmp("PWD", str) == MATCH)
		return (g_global.info.current_dir_path);
	if (get_shell_val && g_global.info.unset_oldpwd_flag == false && ft_strcmp("OLDPWD", str) == MATCH)
