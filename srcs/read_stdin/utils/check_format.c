...
/*   Updated: 2021/04/09 12:31:28 by tjinichi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	check_pipe(t_string *command, t_minishell *info)
{
	...
	if (pipe_ret == FIRST_PIPE)
		...
		if (waiting_for_next_command(command, is_valid_command_pipe, info) == false)
			return (false);
	return (true);
}
...
static bool	check_slash(t_string *command, t_minishell *info)
{
	...
	if (is_valid_command_slash(command->str) == false)
		if (waiting_for_next_command(command, is_valid_command_slash, info) == false)
			return (false);
	return (true);
}
